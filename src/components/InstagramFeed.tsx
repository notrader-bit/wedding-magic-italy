...
import { ZoomableImage, type GalleryImage } from "@/components/ZoomableImage";
...
export function InstagramFeed({ eyebrow, title, viewPhotoLabel, posts }: Props) {
  const gallery = useMemo<GalleryImage[]>(
    () => posts.map((post) => ({ src: post.imageUrl, alt: post.alt, lightboxSrc: post.imageUrl })),
    [posts],
  );
